...
void LendoArquivo(char nome[],char tipo[])
{
    ...
    return;
		 ...
		 while((ch=fgetc(fp))!=EOF)
		 {
		    ...
		    if(ch==EOF)
		    { //se encontrou o final do arquivo
		    //printf("\nChegou ao final");
		    }
		}
	getchar();
...
void CriaArquivo(char nome[], char tipo[])
{
	...
	{
        printf("Digite as informações do %s",tipo);
        ...
        fprintf(fp,"\n%d\n%s\n%s",autor.idAutor,autor.nomeAutor,autor.sobrenomeAutor);
        }
        ...
        scanf("%s",livro.isbn);
         ...
         fprintf(fp,"\n%d\n%d\n%s\n%s\n%s",livro.idLivro,livro.anoPublicacao,livro.titulo,livro.editora,livro.isbn);
        }
        ...
        scanf("%i",&leitor.estado);
         ...
         fprintf(fp,"\n%d\n%s\n%s\n%d\n%d\n%d",leitor.idLeitor,leitor.nome,leitor.fone,leitor.endereco,leitor.cidade,leitor.estado);
        ...
        fprintf(fp,"\n%d\n%d\n%d",autordolivro.autorID,autordolivro.livroID,autordolivro.sequence);
        }
        else
        {
...
void LendoNaMemoria(char nome[],char tipo[])
{
	...
	char ch,*p_ch;
	p_ch=&ch;
	int cont=0,i=0,valores=0,mem,stopstr=0;
	char str[20];
	FILE *fp = fopen(nome,"r");
	...
	while((ch=fgetc(fp))!=EOF)
		 {
				...
				stopstr=0;
		  		...
		  		{
		  		str[stopstr+1]='\0';
		  		strcat(autor[i].nomeAutor,str);
		  		}
		  		else if(valores==3) //para ler o Sobrenome
		  		{
		  		str[stopstr+1]='\0';
		  		strcat(autor[i].sobrenomeAutor,str);
		  		i++;
		  		}
	...
	printf("Valor de ID autor: %d",autor[i].idAutor);
	printf("Valor de Nome autor: %s",autor[i].nomeAutor);
	printf("Valor de Sobrenoome autor: %s",autor[i].sobrenomeAutor);
	}
