...
/***************************************************************************************/
/* FEITO PELA EQUIPE: BRUNA DE OLIVEIRA */
...
void ordena(char *nome){
    ...
    int tam_arq=0, tam_arq_aux=0,i,j,numero, count=0, ii=0, pos;
    arqGeral=fopen(nome,"rb");
    fseek(arqGeral,0,SEEK_END);
    tam_arq=ftell(arqGeral);
    rewind(arqGeral);
...
//////////////Só verifica se está escrevendo certo os indices //////////////////////////

/// ** Não esta escrevendo certo
void mostraIndex(char *nome){
    char nomea[100] = "\0";
    heap aux;
    strcpy(nomea,nome);
